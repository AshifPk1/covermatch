...
class InsurancePremium(models.Model):
    ...
    insurance_premium = fields.Selection([('thirdparty', 'Third Party'), ('comprehensive', 'Comprehensive')],
    ...
    premium_product = fields.Many2one('product.product', "Insurance Product", required=True)
    ...
    # driver_age_max = fields.Selection([('18to25', 'Between 18 years to 25 years'), ('25_above', 'Above 25 years')],
    #                                   string="Driver Age Max ")
    driver_aged = fields.Char(string="Driver Age")
    driver_aged_max = fields.Char(string="Driver Age Max")
    date_of_first_registration = fields.Selection(
        [('1st', '1st Year'), ('2nd', '2nd Year'), ('3rd', '3rd Year'), ('4th', '4th Year'), ('5th', '5th Year'),
         ('6th', '6th Year and above')],
    ...
    # benefits
    included_ids = fields.Many2many('res.benefits', 'is_included', string='Included Benefits',
                                    domain=[('is_included', '=', True)])
    excluded_ids = fields.Many2many('res.benefits', 'is_excluded', string='Excluded Benefits',
                                    domain=[('is_excluded', '=', True)])
    optional_ids = fields.Many2many('res.benefits', 'is_optional', string='Optional Benefits',
                                    domain=[('is_optional', '=', True)])

    comprehensive_insurance_ids = fields.One2many('comprehensive.insurance', 'premuium_type_id')
    third_party_insurance_ids = fields.One2many('thirdparty.insurance', 'premuium_type_id')
    # included_models_ids = fields.Many2many('vehicle.model', string='Included Model')
    included_vehicle_ids = fields.Many2many('uae.car.valuation', string='Excluded Vehicle')
    year1 = fields.Float(string="1 Year")
    year2 = fields.Float(string="2 Year")
    year3 = fields.Float(string="3 Year")
