...
class CrmLead(models.Model):
    ...
    insurance_type = fields.Selection([('third_party', 'Third Party'), ('comprehensive', 'Comprehensive')],
                                      string="Current Insurance Type")
    # basic_or_standard = fields.Selection([('basic', 'Basic'), ('standard', 'Standard')],
    ...
    motor_expiry = fields.Date('Insurance Expiry Date')
    ...
    insurance_expiry_date = fields.Selection([(str(num), str(num) + ' ' + 'Years') for num in range(5, 51)],
                                             string='Insurance Expiry Date Old')
    ...
    version_id = fields.Many2one('vehicle.version', "Version")
    ...
    driving_experience_bike = fields.Selection([(str(num), str(num) + ' ' + 'Years') for num in range(5, 51)],
                                               string='Driving Experience Old')
    ...
    claim_status = fields.Selection(
        [('yes', 'Yes'), ('no', 'No')], string='Claim Status',
        help='Last 12 Months Claim Status')
    ...
    make_old = fields.Char('Make Old')
    model_old = fields.Char('Model Old')
    version_old = fields.Char('Version Old')
    ...
    no_of_children = fields.Char('No Of Children')

    # Bike Insurance

    details_of_bike = fields.Char('Details of the bike')
    value_of_insurance = fields.Float('Value Of Insurance')
    ...
    other_vessel_type = fields.Char('Other Vessel Type')
    # manufacture_year = fields.Selection([(str(num), num) for num in range(1900, (datetime.now().year) + 100)], 'Year')
    ...
    is_smoker = fields.Selection(
        [('yes', 'Yes'), ('no', 'No')], string='Smoker')
    how_much_covereage = fields.Char('How much Coverage')
    how_long = fields.Char('How Long To Be Protected For')
    insurance_for = fields.Selection(
        [('self', 'Yourself'), ('spouse', 'Yourself and Spouse')], string='Life Insurance For')
    ...
    def action_new_quotation(self):
        ...
        res['context']['default_Date_of_birth'] = self.Date_of_birth
        res['context']['default_vehicle_id'] = self.vehicle_id.id
        res['context']['default_years_of_no_claim_certificate'] = self.years_of_no_claim_certificate
        return res

    @api.onchange('vehicle_id')
    def _onchange_vehicle_id(self):
        if self.vehicle_id:
            self.make_id = self.vehicle_id.make_id
            self.model_id = self.vehicle_id.model_id
            self.version_id = self.vehicle_id.version_id
            self.year = self.vehicle_id.year
...
class AdditionalMembers(models.Model):
    ...
    relationship = fields.Selection([('self', 'Self'), ('child', 'Child'), ('spouse', 'Spouse'), ('parent', 'Parent')],
                                    string='Relationship')
    gender = fields.Selection([('male', 'Male'), ('femal', 'female')], string='Gender')
...
class InsurancePolicy(models.Model):
    ...
    insurance_company_id = fields.Many2one('res.partner', 'Insurance Company',
                                           domain=[('is_insurance_comapny', '=', True)])
    commission_from = fields.Selection([('broker', 'Broker'), ('company', 'Company')], string='Commisiion From',
                                       required=True)
